...
describe('theme generator', () => {
  ...
  it('falls back to neutral background tones when extInfo omits them', () => {
    ...
    expect(light['--color-app-background']).toBe('rgba(245, 247, 249, 0.55)')
    expect(light['--color-main-background']).toBe('rgba(255, 255, 255, 0.85)')

    const dark = createThemeColors('rgb(85, 196, 158)', undefined, true)
    expect(dark['--color-app-background']).toBe('rgba(18, 21, 23, 0.55)')
    expect(dark['--color-main-background']).toBe('rgba(26, 30, 33, 0.82)')
  })

  it('derives border tiers from the background tone and honors overrides', () => {
    const light = createThemeColors('rgb(22, 121, 97)', undefined, false)
    expect(light['--color-border-subtle']).toBe('rgba(235, 235, 235, 0.85)')
    expect(light['--color-border']).toBe('rgba(209, 209, 209, 0.85)')
    expect(light['--color-border-strong']).toBe('rgba(173, 173, 173, 0.85)')

    const dark = createThemeColors('rgb(85, 196, 158)', undefined, true)
    expect(dark['--color-border-subtle']).toBe('rgba(40, 44, 46, 0.82)')
    expect(dark['--color-border']).toBe('rgba(53, 57, 60, 0.82)')
    expect(dark['--color-border-strong']).toBe('rgba(72, 75, 77, 0.82)')

    const overridden = createThemeColors('rgb(22, 121, 97)', undefined, false, false, {
      '--color-border': '#123456',
    })
    expect(overridden['--color-border']).toBe('#123456')
    expect(overridden['--color-border-subtle']).toBe('rgba(235, 235, 235, 0.85)')
  })
